...
<div class="min-h-screen flex items-center justify-center px-[20px] py-[40px]">
    <div class="w-[450px] max-w-full rounded-[20px] bg-white/15 p-[35px] text-center text-white shadow-[0_0_20px_rgba(0,0,0,0.2)] backdrop-blur-[10px] max-[520px]:p-6">
        ...
        <p class="mb-6 text-sm text-white/80">
            Masukkan kode OTP yang dikirim ke <strong><?php echo e($email); ?></strong> lalu buat password baru.
        </p>
        ...
        <form action="<?php echo e(session('otp_verified_email') ? route('password.update') : route('password.verify-otp')); ?>" method="POST" class="flex flex-col gap-[18px]">
            <?php echo csrf_field(); ?>

            <input type="text" name="otp" inputmode="numeric" maxlength="6" placeholder="Kode OTP (6 digit)" required
                   class="w-full rounded-[12px] bg-white px-[18px] py-[16px] text-center text-[20px] font-bold tracking-[8px] text-black outline-none" <?php echo e(session('otp_verified_email') ? 'disabled' : ''); ?>>

            <?php if(session('otp_verified_email')): ?>
        ...
        <p class="mt-5 text-sm text-white">
            Tidak menerima kode?
            <a class="font-bold text-[#2cc2d6] hover:underline" href="<?php echo e(route('password.request')); ?>">Kirim ulang</a>
        </p>
...
<?php /**PATH /Applications/XAMPP/xamppfiles/htdocs/detail.pkerjaan.dara/resources/views/reset-password.blade.php ENDPATH**/ ?>