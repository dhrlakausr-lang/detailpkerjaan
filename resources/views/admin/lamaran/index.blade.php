    ...
    <main class="mx-auto max-w-[1280px] px-6 py-8">
        ...
        <section class="mb-6 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            <form method="GET" action="{{ route('admin.lamaran.index') }}" class="grid gap-3 md:grid-cols-[1fr_220px_auto]">
                ...
                <select name="status" class="rounded-lg border border-slate-300 px-4 py-3 outline-none focus:border-[#2563eb] focus:ring-2 focus:ring-[#2563eb]/20">
                                ...
                                <td class="px-5 py-4">
                                    <div class="flex flex-col gap-2">
                                        <form action="{{ route('admin.lamaran.status', $lamaran) }}" method="POST">
                                            ...
                                            <button class="w-full rounded-lg bg-green-600 px-4 py-2 text-xs font-bold text-white transition hover:bg-green-700">Terima</button>
                                        </form>
                                        <form action="{{ route('admin.lamaran.status', $lamaran) }}" method="POST">
                                            @csrf
